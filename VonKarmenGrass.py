...
Datapoints = 4
heights = np.array([0.46,1.95,0.46,1.95])
#%%
#Loading data and isnerting numpy nans
data=np.loadtxt("day2-1Data.csv", delimiter = ',', encoding='utf-8-sig')
for i in range(len(data[0,:])):
    for j in range(len(data[:,i])):
        if data[j][i] < 0:
            data[j][i] = np.nan

temps=np.loadtxt("day2-1Temps.csv", delimiter = ',', encoding='utf-8-sig')
for i in range(len(data[0,:])):
...
for j in range(len(data[:,0])):
    uprimelow.append(averages[0]-data[j,0])
    
uprimehigh =[]
for j in range(len(data[:,1])):
    uprimehigh.append(averages[1]-data[j,1])

wprimelow =[]
for j in range(len(data[:,2])):
    wprimelow.append(averages[2]-data[j,2])

wprimehigh =[]
for j in range(len(data[:,3])):
    wprimehigh.append(averages[3]-data[j,3])
    
plt.figure(3)
plt.plot(time,uprimelow)
plt.plot(time,uprimehigh)
plt.plot(time,wprimelow)
...
#     # plt.plot((frequencies*heights[i]/average_list[i]), average_list[i]* savgol_filter(abs(x),45,2)/(heights[i]*np.var(interpolated_data[i])))
...
# plt.title("Plot ū(z) against ln(z)")
